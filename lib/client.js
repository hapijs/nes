...
(function (root, factory) {
    ...
    const parse = function (message) {
        ...
        try {
            ...
            throw new NesError(err, 'protocol');
        }
    };
    ...
    Client.prototype.connect = function (options) {
        ...
        if (this._reconnection) {
            return Promise.reject(new NesError('Cannot connect while client attempts to reconnect', 'user'));
        }

        if (this._ws) {
            return Promise.reject(new NesError('Already connected', 'user'));
        }
    ...
    Client.prototype._connect = function (options, initial, next) {
        ...
        ws.onerror = (event) => {
            ...
            if (!ws.onopen &&    // ignore node network errors after open
                event.errno) {
        ...
        ws.onclose = (event) => {
            ...
            if (ws.onopen) {
                finalize(new NesError('Connection terminated while waiting to connect', 'ws'));
            }
    ...
    Client.prototype._send = function (request, track) {
        ...
        if (!this._ws ||
            ...
            return Promise.reject(new NesError('Failed to send message - server disconnected', 'disconnect'));
        }
        ...
        if (!track) {
            try {
                ...
                return Promise.reject(new NesError(err, 'ws'));
            }
        }
        ...
        if (this._settings.timeout) {
            record.timeout = setTimeout(() => {
                ...
                record.timeout = null;
                return record.reject(new NesError('Request timed out', 'timeout'));
    ...
    Client.prototype.subscribe = function (path, handler) {
        ...
        if (!path ||
            ...
            return Promise.reject(new NesError('Invalid path', 'user'));
        }
    ...
    Client.prototype._onMessage = function (message) {
        ...
        next(new NesError('Received invalid response', 'protocol'));
