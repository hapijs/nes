...
(function (root, factory) {
    ...
    var Client = function (url, options) {
        ...
        this._subscriptions = {};                   // criterion -> [callbacks]
    ...
    Client.prototype.subscribe = function (criterion, handler) {
        ...
        if (!criterion ||
            ...
            return handler(new Error('Invalid criterion'));
        }

        var subs = this._subscriptions[criterion];
        if (subs) {
        ...
        this._subscriptions[criterion] = [handler];
    ...
    Client.prototype.unsubscribe = function (criterion, handler) {
        ...
        var sync = false;
        if (!handler) {
            delete this._subscriptions[criterion];
            sync = true;
        }
        else {
    ...
    Client.prototype._onMessage = function (message) {
        ...
        parse(message.data, function (err, update) {
            ...
            if (update.nes === 'pub') {
                var handlers = self._subscriptions[update.criterion];
                if (handlers) {
            ...
            if (update.nes === 'sub') {
                ...
                if (handlers) {
                    for (var i = 0, il = handlers.length; i < il; ++i) {
                        handlers[i](new Error(update.error));                               // Response means error
                    }

                    delete self._subscriptions[update.criterion];                          // Error means no longer subscribed
                }
