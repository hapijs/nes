...
var internals = {
    defaults: {
        auth: {
            ...
            // route: {},                       // hapi route auth configuration
            // password: 'password',            // Credentials encryption password
            cookie: 'nes',                      // Cookie name
            isSecure: true,
            isHttpOnly: true,
            path: '/'
        }
...
internals.authSchema = Joi.object({
    ...
    type: Joi.valid('cookie', 'token').required(),
    route: Joi.object(),
...
internals.auth = function (server, settings) {
    ...
    Joi.assert(settings, internals.authSchema, 'Invalid authentication configuration');
