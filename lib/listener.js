...
internals.Listener.prototype._eachSocket = function (each, options) {
    ...
    route.subscribers._forEachSubscriber(sub.paramsArray.length ? options.subscription : null, {}, (socket) => each(socket));   // Filter on path if has parameters
};
...
internals.Subscribers.prototype.remove = function (socket, path) {
    ...
    if (!path) {
        if (this._settings.onUnsubscribe) {
            for (let i = 0; i < item.paths.length; ++i) {
                this._settings.onUnsubscribe(socket, item.paths[i], item.params[i]);
            }
        }
    ...
    if (this._settings.onUnsubscribe) {
        this._settings.onUnsubscribe(socket, path, params);
    }
};
