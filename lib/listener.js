...
internals.Listener.prototype._add = function (ws) {
    ...
    ws.once('close', function (code, message) {
        ...
        if (self._settings.onDisconnection) {
            self._settings.onDisconnection(socket);
        }
    });
};
...
internals.Listener.eachSocket = function (callback, options) {
    ...
    for (var i = 0, il = connections.length; i < il; ++i) {
        var connection = connections[i];
        if (connection.plugins.nes) {
            connection.plugins.nes._listener._eachSocket(callback, options);
        }
    }
};
...
internals.Listener.prototype._eachSocket = function (callback, options) {
    ...
    if (!subscription) {
        return this._sockets.forEach(callback);
    }
