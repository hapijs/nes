...
internals.subSchema = Joi.object({
    ...
    onSubscribe: Joi.func(),                                        // function (socket, path)
...
internals.Listener.prototype._publish = function (path, update, options) {
    ...
    route.subscribers.forEach(match.paramsArray.length ? path : null, (socket) => {       // Filter on path if has parameters

        if (!route.filter) {
            return socket._send(update, Hoek.ignore);               // Ignore errors
        }

        route.filter(path, update.message, { credentials: socket.auth.credentials, params: match.params, internal: options.internal }, (isMatch) => {
...
internals.Listener.prototype._subscribe = function (path, socket, next) {
    ...
    match.route.subscribers.add(socket, path, match);
...
internals.Subscribers.prototype.add = function (socket, path, match) {
    ...
    this._onSubscribe(socket, path, match.params);
};
