...
internals.Listener.prototype._beat = function () {
    ...
    if (this._heartbeat &&                              // Skip the first time
        ...
        this._sockets._forEach((socket) => {
            ...
            socket._send(update, Hoek.ignore);          // Ignore errors
        });
...
internals.Listener.prototype._broadcast = function (update, options) {
    ...
    const each = (socket) => socket._send(update, Hoek.ignore);     // Ignore errors
...
internals.subSchema = Joi.object({
    ...
    onUnsubscribe: Joi.func(),                                      // function (socket, path)
    auth: Joi.object({
...
internals.Listener.prototype._publish = function (path, update, options) {
    ...
    route.subscribers._forEach(match.paramsArray.length ? path : null, (socket) => {                     // Filter on path if has parameters

        if (!route.filter) {
            return socket._send(update, Hoek.ignore);                           // Ignore errors
        }
        ...
        route.filter(path, update.message, { socket, credentials: socket.auth.credentials, params: match.params, internal: options.internal }, (isMatch, override) => {
            ...
            if (isMatch) {
                ...
                return socket._send(copy, Hoek.ignore);                         // Ignore errors
            }
        });
    });
};
...
internals.Listener.prototype._eachSocket = function (each, options) {
    ...
    route.subscribers._forEach(sub.paramsArray.length ? subscription : null, (socket) => each(socket));   // Filter on path if has parameters
};
...
internals.Subscribers.prototype.add = function (socket, path, match, next) {
    ...
    const add = (err) => {
        ...
        if (item) {
            ...
            this._items[socket.id] = { socket: socket, paths: [path], params: [match.params] };
        }
    ...
    this._onSubscribe(socket, path, match.params, add);
};
...
internals.Subscribers.prototype.remove = function (socket, path) {
    ...
    if (!path) {
        if (this._onUnsubscribe) {
            for (let i = 0; i < item.paths.length; ++i) {
                this._onUnsubscribe(socket, item.paths[i], item.params[i]);
            }
        }
    ...
    if (this._onUnsubscribe) {
        this._onUnsubscribe(socket, path, params);
    }
};


internals.Subscribers.prototype._forEach = function (path, each) {
