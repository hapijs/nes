...
internals.Listener.publish = function (path, update, options) {
    ...
    this.plugins.nes._listener._publish(path, message, options);
};
...
internals.Listener.prototype._publish = function (path, update, options) {
    ...
    route.subscribers._forEachSubscriber(match.paramsArray.length ? path : null, options, async (socket) => {       // Filter on path if has parameters
    ...
    if (!path) {
        ...
        if (this._settings.onUnsubscribe) {
            for (let i = 0; i < item.paths.length; ++i) {
                ...
                try {
                    await this._settings.onUnsubscribe(socket, itemPath, item.params[i]);
                }
                catch (ignoreErr) { }
            };
