...
internals.Socket.prototype._verifyAuth = function () {

    if (!this.auth.isAuthenticated) {
        // no credentials available - no need to verify them
        return true;
    }

    if (Date.now() - this.auth._verified < this._listener._settings.auth.verify) {
        // credentials checked fairly recently - save some CPU cycles
        return true;
    }

    // @todo: the below should probably become this.server.auth.verify()?
    if (!this.server.auth._strategies.default.methods.verify) {
        return true;
    }

    this.auth._verified = Date.now();
    return this.server.auth._strategies.default.methods.verify(this.auth.credentials, this.auth.artifacts);
};
