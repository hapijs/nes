...
internals.Socket.prototype.disconnect = function () {
    ...
    clearTimeout(this.auth._expireTimeout);
    this.auth._expireTimeout = null;
...
internals.Socket.prototype._authenticate = async function (request) {
    ...
    if (request.auth) {
        ...
        await this._setCredentials(auth.credentials, auth.artifacts);
    }
};
...
internals.Socket.prototype._setCredentials = async function (credentials, artifacts) {
    ...
    clearTimeout(this.auth._expireTimeout);
    const expiresAt = this._listener._settings.auth.expiresAt && await this._listener._settings.auth.expiresAt(this.auth);
    if (expiresAt) {
        const timeout = expiresAt - Date.now();
        this.auth._expireTimeout = setTimeout(() => this.disconnect(), timeout);
    }
