...
internals.Socket.prototype._onMessage = function (message) {
    ...
    internals.parse(message, function (err, request) {
        ...
        if (!self.auth.isAuthenticated &&
            self._listener._authRequired()) {

            return self._send({ type: request.type, id: request.id, path: request.path, error: 'Connection is not authenticated' });
        }
...
internals.Socket.prototype._processMessage = function (request) {
    ...
    if (!self._listener._settings.onMessage) {
        return self._send(Boom.badData('Custom messages are not supported'), { id: request.id });
    }
...
internals.Socket.prototype._authenticate = function (next) {
